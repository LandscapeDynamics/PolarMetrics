...
#' @description A basic, but fast function for calculating all polar metrics from XTS arrays.
#' @importFrom stats sd
#' @importFrom zoo index
#' @param input A vector of values or a column of an xts object indexed with time stamps.
#' @param t An optional vector of time values (e.g., days) corresponding to the input vector sampling points. If \code{input} is an xts object then omit t argument.
#' @param yr_type Argument specifying either 'cal_yr' for output (of timing variables) given in days starting from Jan. 1, or 'offset_yr' for output in days starting from the average seasonal minimum.
#' @param spc Integer value specifying the number of samples per cycle (measurements per year) in input.
#' @param lcut Numeric value in the range [0,0.5] passed to \code{window_idx} function. Indicates the percentile to truncate from the first half of each cycle. For example, 0.15 will result in remove the interval corresponding to [0\%,15\%] representing a window that begins after the 15th-percentile is crossed.
#' @param hcut Numeric value in the range (0.5,1] passed to \code{window_idx} function. Indicates the percentile to truncate from the last half of each cycle. For example, 0.85 will result in remove the interval corresponding to [85\%,100\%] representing a window that begins after the 85th-percentile is crossed.
#' @param return.vecs logical argument specifying whether or not to include all of the horizontal and vertical component vectors in output.
#' @details \code{calc_metrics} runs through the entire polar transformation process and conveniently outputs the final polar metrics for all years included in the input.
#' @return Returns a list with the derived polar metrics (e.g., length of season). Optional objects in the list are the actual vecturs used in deriving the polar metrics and the average vector angle and magnitude.
...
calc_metrics <- function(input, t=NULL, yr_type, spc, lcut, hcut, return.vecs) {
  ...
  npy <- nyr - 1          # No. of complete years in output
  ...
  av_doy <- rad2d(avec_ang, dpc=dpy) # Cal dy (1-365 beg at Jan 1) marking min
  ...
  for (J in 1:npy) { # Calculate pheno param's for each yr
    ...
    if (yr_type == 'cal_yr') {
      ...
      output$es[J] <- round((es + av_doy) %% dpy)   # DOY marking ES milestone
      output$ems[J] <- round((ems + av_doy) %% dpy) # DOY marking EMS milestone
      output$ms[J] <- round((ms + av_doy) %% dpy)   # DOY marking MS milestone
      output$lms[J] <- round((lms + av_doy) %% dpy) # DOY marking MLS milestone
      output$ls[J] <- round((ls + av_doy) %% dpy)   # DOY marking LS milestone
  ...
  if (return.vecs == FALSE) {
    ...
    output <- list('metrics' = output, 'vectors' = cbind(VX, VY),
                   'avg.vectors' = cbind(rv_ang, avec_ang,
                                         av_doy, rv_mag))
    return(output)
  }
}
